...
def main():
    parser = argparse.ArgumentParser(description="Train LoRA on Leonardo DiCaprio dataset using PEFT")
    ...
    # Load dataset
    images, texts = load_dataset(args.dataset_dir)
    ...
    for epoch in range(math.ceil(args.max_train_steps / len(images))):
        unet.train()
        total_loss = 0
        ...
        for step in range(0, len(images), args.train_batch_size):
            batch_indices = indices[step:step + args.train_batch_size]
            ...
            for idx in batch_indices:
                # Resize image
                image = images[idx].resize((args.resolution, args.resolution))
            ...
            # Stack images
            pixel_values = torch.cat(batch_images, dim=0).to(accelerator.device)
        ...
        # Log progress
        avg_loss = total_loss.item() / (len(images) // args.train_batch_size)
    ...
    unwrapped_unet.save_pretrained(os.path.join(args.output_dir, "lora"))
